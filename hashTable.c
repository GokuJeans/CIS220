...
int hash(char *key) {
	int stringHash = 0;  // Set initial value to 0
    int multiplier = 13;  // Set multiplier as 13
    while (*key) {  // Go through each character in the key
        stringHash = stringHash * multiplier + *key;  // Update hash value
        key++;  // Move to the next character
    }
    return stringHash; // Return the hash value 
...
void printTable(HashTable *table) {
    for (int i = 0; i < table->length; i++) {  
        ...
        if (currentItem == NULL) {  
            ...
            while (currentItem != NULL) {
                int hashValue = hash(currentItem->key);  // Calculate the hash value for each key
...
int main() {
    ...
    if (hashRemove(table, "keyF52")) {    	// 8. Remove keyF52. If the key was found, displays "keyF52 was removed", otherwise, "keyF52 was not found"
    ...
    if (value) 					{						// 9. Attempt to get a non-existing key’s value: keyZ6
        printf("Key found: %s\n", value);
    } else {
    ...
    value = hashGet(table, "keyC3");      	// 10. Get an existing key: keyC3.
    if (value) {
        printf("%s\n", value);
    } else {
	...
	hashInsert(table, "keyD23", "newvalue23");	// 14. Update keyD23 value to newvalue23.
